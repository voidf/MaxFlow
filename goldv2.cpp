...
struct GoldbergRao
{
    ...
    // const
    inline bool is_at_capacity(int u, int v) const { return get_residual_cap(u, v) == 0; }

    inline T get_residual_cap(int u, int v, bool include_reverse_flow = true) const
    {
    ...
    ContractedGraph condensation()
    {
        ...
        vec<us<int>::iterator> cur; // 非递归scc用当前弧，可以干掉看看是不是更快
        vec<int> sta;
        ...
        for (int i = 0; i < N; ++i)
        {
            ...
            while (sta.size())
            {
                ...
                int v = sta.back();
                bool done = 1;
                while (cur[v] != len0[v].end())
                // while (cur[v] != R.E[v].end())
                {
                    // ++ctr_sccedge;
                    const auto w = *cur[v];
                    const auto &a = R.E[v][w];
                    // auto [w, a] = *cur[v];
                    if (dfrom[v] != INF)
                    {
