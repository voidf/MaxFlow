...
struct cached_edge
{
    cached_edge(T to, U c) : to(to), cap(c), rcap(c) {}
    T to;
    cached_edge *rev;
    U cap, rcap;
};
    ...
    private:
        ...
        void init()
        {
            ...
            for (auto &edge : rnet[_source]) // 先把流从源点提出来
            {
                ...
                edge.rcap += edge.cap;
                edge.rev->cap += edge.cap;
                edge.rev->rcap -= edge.cap;
                edge.cap = 0;
            }
            hivert = 1;
        }
...
struct Solver
{
    ...
    void add_edge(int u, int v, U w)
    {
        ...
        G[u].back().rev = (cached_edge<int, U> *)(G[v].size() - 1);
        G[v].back().rev = (cached_edge<int, U> *)(G[u].size() - 1);
    }
