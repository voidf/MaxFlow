...
{
    template <class T>
    static size_t h(size_t s, const T &v) { return s ^ (hash<T>()(v) + 0x9e3779b9 + (s << 6) + (s >> 2)); }
...
struct GoldbergRao
{
            ...
            if (scc_found[s] == 0)
            {
                ...
                while (Q.size())
                {
                    ...
                    if (done)
                    {
                        ...
                        {
                            us<int> scc({v});
                            while (scc_queue.size() && preorder[scc_queue.back()] > preorder[v]) /**/
                            {
                                scc.emplace(scc_queue.back());
                                scc_queue.pop_back();
                            }
                            for (auto u : scc)
                                scc_found[u] = 1;
                            ret.emplace_back(scc);
                        }
                        else
                            scc_queue.emplace_back(v);
                    }
                }
            }
        return ret;
    ...
    ContractedGraph construct_graph_contraction(int start_node, int end_node)
    {
        ...
        for (int scc = 0; scc < condensed_graph.E.size(); ++scc)
        {
            ...
            condensed_graph.representative[scc] = rep_vertex;
            // out tree
